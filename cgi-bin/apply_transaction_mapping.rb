...
begin
	...
	sel_stmt = "select transaction_date,imp.src_bank_account,imp.description,amount,src_line_type_id,dst_line_type_id,src_account_id,dst_account_id,upload_date from imported_data as imp left join correlations_table as ct on imp.src_bank_account = ct.src_bank_account and imp.description = ct.dst_description left join uploaded_files as uf on imp.upload_id = uf.upload_id where imp.correlated_flag <> 1 or imp.correlated_flag is null;"
	
	book_id = "1" # TODO: How to get?
	entry_id = 1
	entry_id = dbh.select_one("select ifnull(max(entry_id)+1, 1) as nextID from accounting_entries;").to_s.to_i
	...
	update_correlate = "update imported_data left join correlations_table on imported_data.src_bank_account = correlations_table.src_bank_account and imported_data.description = correlations_table.dst_description set imported_data.correlated_flag = 1 where imported_data.correlated_flag <> 1 or imported_data.correlated_flag is null;" 
